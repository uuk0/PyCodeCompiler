...
#define PY_THROW_EXCEPTION_IF_WITH_MESSAGE_AND_OBJ(condition, exception, message, obj) if (condition) { printf(message, PY_getObjectRepr(obj)); return PY_STD_wrapException(exception); }

#else

#define PY_THROW_EXCEPTION(exception) { fprintf(stderr, "%s\n", PY_getObjectRepr(exception)); int* x = NULL; int y = *x; return NULL; } // assert(false);
